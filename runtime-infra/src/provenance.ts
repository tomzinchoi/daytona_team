...
export const reporterSource = readFileSync(resolve(root, 'runtime/test-reporter.mjs'));
...
export const runnerHash = createHash('sha256').update(runnerSource).update(reporterSource).digest('hex');
export function emptyResult(request: RunRequest, runId: string, timeoutSeconds: number, maxTokens = 512): BenchmarkResult {
  return {
    ...
    evaluator: null, caseEvidence: null, output: null, agents: [],
    provenance: { sandboxId: null, snapshotId: null, environmentHash: null, fixtureHash: hash(request.benchmarkCase), architectureHash: hash(request.architecture), runnerHash, sourceArchitectureHash: request.architecture.sourceArchitectureHash ?? null, maxTokensPerAgent: maxTokens, timeoutSeconds, resources: null, inference: 'local-llama.cpp', modelManifest: null },
