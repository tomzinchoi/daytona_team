...
import { FILE_ACCEPT, mergeWorkloadFiles, readWorkloadFiles, type WorkloadFile } from '../workload-files';
...
export default function WorkloadUpload({ files, onChange, onReading }: { files: WorkloadFile[]; onChange: (files: WorkloadFile[]) => void; onReading: (reading: boolean) => void }) {
  ...
  async function select(selected: File[]) {
    if (readInProgress.current || !selected.length) return;
    readInProgress.current = true;
    const token = ++generation.current;
    setError(''); setReading(true); onReading(true);
    try { const parsed = await readWorkloadFiles(selected); if (token === generation.current) onChange(mergeWorkloadFiles(files, parsed)); }
    catch (e) { if (token === generation.current) setError(e instanceof Error ? e.message : '파일을 읽을 수 없습니다.'); }
    finally { if (token === generation.current) { readInProgress.current = false; setReading(false); onReading(false); } if (input.current) input.current.value = ''; }
  }
  return <div className="workload-upload">
    <div className={`upload-zone ${dragging ? 'dragging' : ''}`} onDragOver={e => { e.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={e => { e.preventDefault(); setDragging(false); void select(Array.from(e.dataTransfer.files)); }}>
      ...
      <input ref={input} type="file" className="sr-only" aria-label="워크로드 파일 업로드" accept={FILE_ACCEPT} multiple disabled={reading} onChange={e => { if (e.target.files?.length) void select(Array.from(e.target.files)); }} />
    ...
    {files.map(file => <div className="uploaded-file" key={file.name}>
      ...
      <button type="button" disabled={reading} aria-label={`${file.name} 삭제`} onClick={() => { setError(''); onChange(files.filter(item => item.name !== file.name)); }}><X size={16} /></button>
    </div>)}
  </div>;
}
